...
async def contentCrawler(Words: list, gallId: str, dataNum: str, firstUrl: str, now: datetime, period: int):
    ...
    async with httpx.AsyncClient() as client:
        while 1 :
            ...
            try:
                ...
                print(f'URL에러! 서버와 통신이 안 됨! 강제종료!: {e}')
                return -1
            else:
                current_no -=1
...
async def firstListParsing(initUrl: str, now: datetime, previousDays: int):
    errorPoint = 0
    count = 1
    ...
    async with httpx.AsyncClient() as client:
        ...
        while 1:
            ...
            try:
                ...
                print(f'해당하는 날짜의 게시글 찾는 중... 반복횟수: {count}번, 현재 dataNum: {dataNum}, 에러횟수: {errorPoint}번')
...
async def startCrawler(initUrl:str, days: int, previousDays: int = 0):
    now = datetime.now()
    ...
    fLP = await firstListParsing(initUrl, now, previousDays)
    if fLP==-1:
        print('크롤링 실패! 프로세스를 종료합니다.')
        return     
    # 마지막인자는 {prviousDays} 일 후 {period} 일 기간 동안의 데이터를 크롤링한다는 의미
    await contentCrawler(Words, fLP[0], fLP[1], fLP[2], now, period+previousDays)
    return 
